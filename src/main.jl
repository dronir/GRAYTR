...
light_col = RGBSpectrum(1.0, 1.0, 1.0)
light_pos = translation(4.0, 4.0, -4.0)
light = PointLight(light_col, light_pos, 1)

scene = Scene(stuff, LightSource[light])

# Make a camera
resX = 256
resY = 256
F = ImageFilm(resX, resY, TriangleFilter(2.0, 2.0))
shift = translation(0.0, 0.0, -2.0)
Cam = OrthographicCamera(shift, [-1.2, 1.2, -1.2, 1.2], 0.0, 0.0, F)

# Make the renderer
sampler = StratifiedSampler(1, resX, 1, resY, 3, 3, 4, true)
